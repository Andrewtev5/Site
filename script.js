...
<button class="search-trigger" id="searchButton" type="button" onclick="openSearchModal()" data-i18n-aria-label="aria.openSearch" aria-label="Open product search">
...
<section class="modal-window search-modal" id="searchModal" aria-hidden="true" data-i18n-aria-label="aria.searchWindow" aria-label="Product search window">
<div class="modal-head">
<div>
<p class="modal-kicker" data-i18n="search.kicker">Wyszukiwanie</p>
<h3 class="modal-title" data-i18n="search.title">Znajdz lampe</h3>
</div>
<button class="modal-close" type="button" onclick="closeActiveModal()" data-i18n-aria-label="aria.closeSearch" aria-label="Close product search">&times;</button>
</div>
...
<span class="search-input-label" data-i18n="search.inputLabel">Nazwa produktu</span>
<input class="search-input" id="searchInput" type="text" autocomplete="off" data-i18n-placeholder="search.placeholder" placeholder="Wpisz nazwe lampy">
</label>
...
score = Math.max(score, weights.startsWith);
}
...
if(tokenMatches){
score = Math.max(score, weights.includes + tokenMatches * weights.tokenBonus);
}

if(query.length >= 3 && isSubsequenceMatch(query, normalizedValue)){
score = Math.max(score, weights.subsequence);
}
});
...
.map((productId) => ({
product: getCatalogProduct(productId),
score: getProductSearchScore(productId, rawQuery)
}))
.filter((entry) => entry.product && entry.score > 0)
.sort((left, right) => right.score - left.score || left.product.name.localeCompare(right.product.name, getLocale()));
}
...
if(!searchInput || !searchResults || !searchResultsMeta){
return;
}
...
<strong>${escapeHtml(t("search.initialTitle"))}</strong>
<p>${escapeHtml(t("search.initialText"))}</p>
</div>
`;
return;
}
...
searchResults.innerHTML = `
<div class="search-empty-state">
<strong>${escapeHtml(t("search.noResultsTitle"))}</strong>
<p>${escapeHtml(t("search.noResultsText", { query: rawQuery }))}</p>
</div>
`;
return;
}
...
<p class="search-result-tag">${escapeHtml(product.tag)}</p>
</div>
<strong class="search-result-price">${escapeHtml(product.price)}</strong>
</div>
...
if(addButton){
addToLibrary(buildProductPayload(addButton.dataset.productId || ""));
renderSearchResults();
return;
}

if(buyButton){
buyProduct(buildProductPayload(buyButton.dataset.productId || ""), "catalog");
}
});
}
}
